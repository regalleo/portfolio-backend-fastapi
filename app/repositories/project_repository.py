...
class ProjectRepository:
    ...
    async def get_all(self) -> List[Project]:
        ...
        return [self._document_to_project(doc) for doc in documents]

    async def get_by_id(self, id: str) -> Optional[Project]:
        try:
            document = await self.collection.find_one({"_id": ObjectId(id)})
            return self._document_to_project(document) if document else None
        except Exception:
            return None
    ...
    async def get_featured(self) -> List[Project]:
        documents = await self.collection.find({"featured": True}).to_list(length=100)
        return [self._document_to_project(doc) for doc in documents]

    async def create(self, project_data: ProjectCreate) -> Project:
        document = project_data.model_dump(exclude_none=True)
        result = await self.collection.insert_one(document)
        document["_id"] = result.inserted_id
        return self._document_to_project(document)
    ...
    def _document_to_project(self, document: dict) -> Project:
        if document:
            document["id"] = str(document.pop("_id"))
            if document.get("completed_date"):
                document["completed_date"] = str(document["completed_date"])
